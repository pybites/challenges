...
def test_find_double_words():
    expected = 'the the'
    assert regex.find_double_words() == expected
...
def test_match_ip_address():
    ...
    for valid_ip in valid_ips:
        assert regex.match_ip_v4_address(valid_ip)
    for bad_ip in bad_ips:
        assert regex.match_ip_v4_address(bad_ip) is None
