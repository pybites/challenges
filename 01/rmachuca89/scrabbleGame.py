    ...
    for countThree in range(len(wordList)):
        ...
        for countFour in range(len(letterList)):
            ...
            for countFive in range(len(stringList)):
                if stringList[countFive] == wordList[countThree].upper(
                ):  # Quickly realised the system has to recognise upper and lower case.
...
def dictTopWord(dict):
    ...
    scoreList = [
    ]  # Create empty scoreList to run concurrently with my dictionary list.
    # Using this two dimensional array I can find the top score and print the relavent index.
    with open(dict, 'r') as f:
        list = f.readlines(
        )  # The following creates a list by reading each line of my .txt file.
        list = [
            x.strip('\n') for x in list
        ]  # This list comprehension is required to remove the '\r' from the list.

    for count in range(
            len(list)
    ):  # Now simply populate my scoreList with each corresponding word in the dictionary list.
...
def main():
    ...
    print('------------------------------------------------')
    print('')
    print('First of all lets give it a test, choose five words and find out \
        what Scrabble scores they have.')
    ...
    if testInput == 'y':
        for count in range(len(testWords)):
            print('--------')
            print('The score of word: ' + str(testWords[count]) + ' is: ' +
                  str(calcWordValue(testWords[count])))
        print('')
        print('--------------------------')
    ...
    print('')
    print(
        'I was given a massive dictionary of words, and my task was to find the highest \
            value words in the dictionary '
    ...
    if dictInput == 'y':
        ...
        print('')
        print(
            'This will take a few minutes, bear with me, the list of words was massive!'
        )
        print('')
        print('===================')
