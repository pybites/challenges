...
def percentage_of_players_first_year():
    """Return 2 digit percentage of players whose first year it is
       (first_year column)"""
    sql = f'SELECT 100.0 * COUNT(*) / (SELECT COUNT(*) FROM players) ' \
          f'FROM players WHERE first_year=1'
...
def year_with_most_drafts():
    ...
    sql = 'SELECT year,COUNT(year) as occurence FROM players GROUP BY year ' \
...
def most_games_per_year_for_veterans():
    """Top 6 players that are > 10 years active, that have the
       highest # games / year"""
    sql = 'SELECT name FROM players WHERE active > 10 ORDER BY 100*games/active DESC LIMIT 6'

    result = cur.execute(sql)

    return [player[0] for player in result.fetchall()]
