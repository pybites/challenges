...
def player_with_max_points_per_game():
    ...
    cur.execute('''SELECT name 
                   FROM players 
                   WHERE CAST(avg_points AS FLOAT) =
                    (SELECT MAX(CAST(avg_points AS FLOAT))
                     FROM players
                    )
                    ''')
...
def percentage_of_players_first_year():
    """Return 2 digit percentage of players whose first year it is
       (first_year column)"""
    cur.execute('''SELECT ROUND(first * 100.0 / (SELECT COUNT(*) 
                   FROM players),2) 
                    FROM (SELECT COUNT(*) first 
                        FROM players WHERE first_year="1"
                         )
                        ''')
    return cur.fetchone()[0]
...
def year_with_most_drafts():
    ...
    cur.execute('''SELECT year FROM
                   (SELECT COUNT(year) yc, year
                    FROM players
                    GROUP BY year
                    HAVING yc =
                        (SELECT MAX(y) FROM 
...
def most_games_per_year_for_veterans():
    ...
    cur.execute('''SELECT name
                    FROM(
                        SELECT name, CAST(games as FLOAT) / CAST(active AS FLOAT) gpy
                        FROM players
