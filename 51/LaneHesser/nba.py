...
def import_to_db(players=None):
    ...
    # you code ...
    cur.execute("""CREATE TABLE players (name, year, first_year, team,
                                         college, active, games, avg_min,
                                         avg_points)""")
    for player in players:
        cur.execute("""INSERT INTO players VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (player.name, player.year, player.first_year, player.team,
                     player.college, player.active, player.games, player.avg_min,
                     player.avg_points))
...
def player_with_max_points_per_game():
    ...
    query = """SELECT name FROM players ORDER BY CAST(avg_points AS NUMERIC) desc LIMIT 1"""
    cur.execute(query)
...
def number_of_players_from_duke():
    ...
    query = """SELECT COUNT(name) FROM players WHERE college = 'Duke University'"""
    cur.execute(query)
    return cur.fetchall()[0][0]
...
def percentage_of_players_first_year():
    """Return 2 digit percentage of players whose first year it is
       (first_year column)"""
    query = """SELECT COUNT(*) FROM players WHERE first_year = '1'"""
    cur.execute(query)
    num_of_first_year_players = cur.fetchall()[0][0]
    query = """SELECT COUNT(*) FROM players"""
    cur.execute(query)
    total_players = cur.fetchall()[0][0]
    return (num_of_first_year_players / total_players) * 100


def avg_years_active_players_stanford():
    """Return the average years that players from "Stanford University"
       are active ("active" column)"""
    query = """SELECT AVG(active) FROM players WHERE college = 'Stanford University'"""
    cur.execute(query)
    return cur.fetchall()[0][0]
...
def year_with_most_drafts():
    ...
    query = """SELECT year, COUNT(year) AS most_drafts
               FROM players
               GROUP BY year
               ORDER BY most_drafts desc"""
    cur.execute(query)
    return cur.fetchall()[0][0]
...
def most_games_per_year_for_veterans():
    ...
    query = """SELECT name
               FROM players
               WHERE CAST(active AS INT) > 10
               ORDER BY (games / active) desc
               LIMIT 6"""
    cur.execute(query)
    result = cur.fetchall()
    return [
