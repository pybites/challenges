...
def get_similarities(tags, similarity_ratio):
    for word_pair in product(tags, tags):
        if word_pair[0][0] == word_pair[1][0]:
            #print(word_pair[0] +" "+ word_pair[1] + "    " + word_pair[0][0] + " " + word_pair[1][0] + " " + str(SequenceMatcher(None, *word_pair).ratio()*100))
            word_pair = tuple(sorted(word_pair))  # set needs hashable type
            similarity = SequenceMatcher(None, *word_pair).ratio()
            if similarity_ratio < similarity < IDENTICAL:
                yield word_pair
...
if __name__ == "__main__":
    ...
    similar_tags = dict(get_similarities(list(tags), get_user_similarity))
    print()
    print('* Similar tags:')
