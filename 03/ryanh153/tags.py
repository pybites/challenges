...
RSS_FEED = 'rss.xml'
SIMILAR = 0.87
# the string in between the category tags if it is 1 or more copies of anything except the '<' character
...
def get_tags():
    ...
    with open(RSS_FEED, 'r') as file:
        for line in file:
            pass
        rss_string = line.lower()

    tags = TAG_HTML.findall(rss_string)
...
def get_top_tags(tags):
    ...
    return Counter(tags).most_common()[0:TOP_NUMBER]
...
def get_similarities(tags):
    ...
    for pair in product(tags, tags):
        ...
        if pair[0] != pair[1] and SequenceMatcher(None, pair[0], pair[1]).ratio() > SIMILAR:
            toReturn.add(tuple(sorted(pair)))
