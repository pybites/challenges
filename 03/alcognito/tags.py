...
def get_tags():
    ...
    with open(RSS_FEED) as xml_file:
        tags = TAG_HTML.findall(xml_file.read().lower())
...
def get_similarities(tags):
    ...
    pairs = sorted(set(product([tag for tag in tags], tags)))
    close_matches = []
    for pair in pairs:
        if pair[0] != pair[1] and pair[0][0] == pair[1][0]:
            if SequenceMatcher(lambda x: x == " ",pair[0], pair[1]).ratio() >= SIMILAR:
                close_matches.append(sorted(pair))
