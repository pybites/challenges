...
def get_top_tags(tags):
    ...
    return sorted(Counter(tags).items(), key=lambda item: item[1], reverse=True)[:TOP_NUMBER]
...
def get_similarities(tags):
    ...
    for tag_1 in tags:
        for tag_2 in tags:
            if tag_1 != tag_2:
                s=SequenceMatcher(lambda x: x == " ", tag_1, tag_2)
                if round(s.ratio(), 2) > SIMILAR:
                    similarities.append(sorted([tag_1, tag_2]))
                    tags.remove(tag_2)
    return similarities
