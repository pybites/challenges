...
class Scrapper:
    ...
    def connect(self, url: str) -> tuple:
        """
    ...
    def scrapper_es(self, html, year: int) -> list:
        """
        ...
        if year == 2018:
            ...
            speakers = self._extract_speakers(items)            
    ...
    @staticmethod
    def _extract_speakers(items: list) -> list:
        speakers = []
        for item in items:
            names = item.text.replace(' y', ',').split(', ')
