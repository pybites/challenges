...
from game import letter_permutations, validate_word, valid_words
...
class TestGame(unittest.TestCase):
    ...
    def test_get_permutations_draw(self):
        ...
        game_permutations = len(list(letter_permutations(self.draw)))
    ...
    def test_get_possible_dict_words(self):
        ...
        words = list(valid_words(letter_permutations(self.fixed_draw)))
        self.assertEqual(len(words), 137)

    def test_validation(self):
        draw = list('garytev'.upper())
        word = 'GARYTEV'
        self.assertRaises(ValueError, validate_word, draw, word)
        word = 'F'
        self.assertRaises(ValueError, validate_word, draw, word)
        word = 'GARETTA'
        self.assertRaises(ValueError, validate_word, draw, word)
