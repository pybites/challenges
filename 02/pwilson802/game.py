...
def input_word(draw):
    ...
    validate = _validation(word, draw)
...
def _validation(word, draw):
    ...
    for letter in word:
        if letter not in draw_copy:
            return [False, 'Letter not in draw']
        else:
            draw_copy.remove(letter)
    if word not in DICTIONARY:
        return [False, 'Letter not in dictionary']
    else:
        return [True]
...
def get_possible_dict_words(draw):
    ...
    return set([x for x in all_permutations if x in DICTIONARY])
...
def _get_permutations_draw(draw):
    ...
    for num in range(1, len(draw)):
        options = itertools.permutations(draw, num)
        for option in options:
            all_permutations.append(''.join(option))
