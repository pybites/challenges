...
def input_word(draw):
    ...
    word = input("Give me a word from the dictionary: ")
    while not _validation(word, draw):
        word = input('Invalid word.  Please select a word from the drawn letters')
    return word
...
def get_possible_dict_words(draw):
    ...
    return [p for p in _get_permutations_draw(draw) if p.lower() in DICTIONARY]
...
def _get_permutations_draw(draw):
    ...
    for n in range(1, NUM_LETTERS+1):
        for i in itertools.permutations(draw, n):
            perms.append("".join(i))
    return perms
